...
def chunk_pages(pages: list[Page], filename: str, doc_id: str) -> list[Chunk]:
    ...
    def flush():
        ...
        if not buf:
            return
        text = " ".join(buf).strip()
        if count_tokens(text) >= CHUNK.min_tokens:
            chunks.append(
                Chunk(
                    text=text,
                    filename=filename,
    ...
    def seed_overlap(emitted: list[str], page_number: int):
        ...
        for s in reversed(emitted):
            t = count_tokens(s)
            if tail_tokens + t > CHUNK.overlap_tokens:
                break
            tail.insert(0, s)
            tail_tokens += t
        buf = tail
    ...
    for page in pages:
        for raw_line in page.text.split("\n"):
            ...
            for sentence in _split_sentences(raw_line):
                ...
                buf_page_end = page.number
                if buf_tokens >= CHUNK.target_tokens:
                    emitted = list(buf)
                    flush()
...
def ingest_file(path: Path, doc_id: str) -> list[Chunk]:
    ...
    return chunk_pages(pages, filename=path.name, doc_id=doc_id)
